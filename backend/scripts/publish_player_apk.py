...
def main() -> None:
    ...
    print(f"  To roll it out to every screen:  python -m scripts.rollout_player {version}")
    print("  (add --at <ISO time> to schedule it). Screens with a live push connection start")
    print("  within seconds; the rest on their next heartbeat (~30s). The install is silent.")
    print("  One screen only: its page in the CMS, Software update.\n")
